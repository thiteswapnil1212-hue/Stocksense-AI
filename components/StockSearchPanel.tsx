...
const MOCK_NEWS: StockNewsItem[] = [
  {
    ...
    summary:
      'Apple exceeded analyst expectations with robust iPhone sales and services revenue.',
  ...
  },
  {
    ...
    provider: 'Yahoo Finance',
    summary:
      "Investors react positively to Apple's artificial intelligence roadmap.",
  ...
  const loadSuggestions = useCallback(
    debounce(async (value: string) => {
      ...
      try {
        ...
        const errorMsg =
          e instanceof Error ? e.message : 'Unable to fetch suggestions';
    ...
    const loadStock = async () => {
      ...
      try {
        const stockRes = await fetch(
          `/api/stocks/${encodeURIComponent(selectedSymbol)}`
        );
        ...
        if (!stockRes.ok) {
          throw new Error(
            stockPayload?.error ||
              `Stock API returned status ${stockRes.status}`
          );
        }

        const newsRes = await fetch(
          `/api/stocks/${encodeURIComponent(selectedSymbol)}/news`
        );
        ...
        const errorMsg =
          e instanceof Error ? e.message : 'Unable to load stock details';
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold text-white">
                  Find ticker data instantly.
                </h2>

                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
                  Start typing a company or symbol, pick the suggestion, and
                  review price charts, key metrics, and latest headlines.
                </p>
              </div>
            </div>
              ...
              <div className="mt-8 min-h-[320px]">
                {loading ? (
                  ...
                  <div className="space-y-8">
                    <div className="grid gap-4 sm:grid-cols-3">
                      <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                        ...
                        <p
                          className={`mt-2 text-sm ${priceTrendColor(
                      ...
                      <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                        ...
                        <p className="mt-3 text-xl font-semibold text-white">
                          {quote.trailingPE?.toFixed(2) ?? 'N/A'}
                        </p>
                    ...
                    <div className="grid gap-4 sm:grid-cols-3">
                      <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                        ...
                        <p className="mt-3 text-sm text-white">
                          {quote.dayLow ?? 'N/A'} - {quote.dayHigh ?? 'N/A'}
                        </p>
                      </div>
                  ...
                  <div className="flex h-full min-h-[320px] items-center justify-center text-slate-400">
                    Search to view stock intelligence.
                  </div>
                )}
              </div>
            </div>
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                        ...
                        <p className="mt-2 text-lg font-semibold text-white">
                          {formatDividendYield(quote?.dividendYield)}
                        </p>
                      </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-3 text-xl font-semibold text-white">
                      News for {quote?.symbol ?? 'your stock'}
                    </h3>
                  </div>
                    ...
                    news.slice(0, 5).map((item) => (
                      ...
                      >
                        ...
                        <p className="mt-2 text-sm leading-6 text-slate-400">
                          {item.summary ??
                            'Read the latest market commentary and updates.'}
                        </p>
                      </a>
                    ))
                  ) : (
          ...
          <div className="glass-card p-6 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <h3 className="mt-3 text-2xl font-semibold text-white">
                  Instant symbol search with smart autocomplete
                </h3>
              </div>
          ...
          <div className="glass-card overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 sm:p-8">
            <div className="flex flex-col gap-4">
              ...
              <p className="text-lg leading-7 text-slate-300">
                Type “app” to see examples like AAPL – Apple Inc. then press
                enter or click the suggestion to load full ticker analysis.
              </p>
